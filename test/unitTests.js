    ...
    test('/api/documents/new', assert => {
      request(app)
        ...
        .end((err, res) => {
          ...
          assert.pass('Created a new document successfully, test 
passed!')
          assert.end()
        })
    })
    ...
    test('/api/documents/:id', assert => {
      request(app)
        ...
        .end((err, res) => {
          if (err) return assert.fail(JSON.stringify(res))
          assert.pass('Got a specific document successfully, test 
passed!')
          assert.end()
        })
    })
    ...
    test('/api/documents/edit/:id', assert => {
      request(app)
        ...
        .send(new Document('test title edit', 'test user edit', 'test 
body edit'))
        .expect(200)
        .end((err, res) => {
          if (err) return assert.fail(JSON.stringify(res))
    ...
    test('/api/documents/delete/:id', assert => {
      request(app)
        ...
        .end((err, res) => {
          if (err) return assert.fail(JSON.stringify(res))
          assert.pass('Deleted a specific document successfully, test 
passed!')
          assert.end()
          done()
        })
